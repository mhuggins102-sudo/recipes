...
2. **Structure** — one Anthropic Messages API call (structured outputs, forced
   JSON schema) turns the recipe into a tree: ingredients are leaves, every
   action is a node combining its children.
...
2. Add `ANTHROPIC_API_KEY` as an **encrypted secret** in the Pages project
   (Settings → Environment variables). It's only ever used server-side in
   `functions/api/convert.ts`.
3. Alternatively deploy from your machine: `npm run deploy`.
...
```
shared/schema.ts        recipe-tree types, Zod validator, API JSON schema, system prompt
